...
export class TabViewComponent implements OnInit {
  ...
  isWorkflowValidationRequired(): boolean {
    return (
  ...
  handleValidationCheck(): void {
    if (
      this.workflowAttributes.validation.required &&
      (!this.template["validation"] || this.template["validation"] === "")
    ) {
      this.validationCheck = false;
      this.messageService.add({
        severity: "error",
        summary: "Validation is required for this action!",
        detail: "Please update Validation for this action in Validation tab",
      });
    } else {
  ...
  executeUpdateActionTemplate(): void {
