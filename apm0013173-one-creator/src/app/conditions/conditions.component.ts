...
export class ConditionsComponent implements OnInit, OnChanges {
  ...
  addCondition(){
    let index;
    if(this.conditionIndexes != []){
      for (let i = 0; i< this.conditionIndexes.length-1; i++){
        if (this.conditionIndexes[i]+1 != this.conditionIndexes[i+1] ){
          index = this.conditionIndexes[i] + 1;
          break;
        }
      }
    }
    this.conditionIndexes.push(index != undefined ? index : this.conditionCount)
    let condition = {allKeys : 'allKeys' +  (index != undefined ? index : this.conditionCount), 
                    condition : 'condition' +  (index != undefined ? index : this.conditionCount), 
                    conditionValue : 'conditionValue' +  (index != undefined ? index : this.conditionCount),
                    alreadyCalled : 'alreadyCalled' + (index != undefined ? index : this.conditionCount),
                    };
    this.formToCheck.addControl('allKeys' + (index != undefined ? index : this.conditionCount), new FormControl('',Validators.required));
    this.formToCheck.addControl('condition' + (index != undefined ? index : this.conditionCount), new FormControl('',Validators.required));
    this.formToCheck.addControl('conditionValue' +(index != undefined ? index : this.conditionCount), new FormControl('',Validators.required));
    this.formToCheck.addControl('alreadyCalled' + (index != undefined ? index : this.conditionCount), new FormControl(false,));
    if (this.conditionCount == 1){this.formToCheck.addControl('type' + (index != undefined ? index : this.conditionCount), new FormControl('',));}
