...
export class EditorComponent implements OnChanges, OnInit, OnDestroy {
  ...
  processTemplateBody(content: string) {
    if (this.form.get('selectedType').value.contentType === ContentTypes.json &&
      ...
      if (content.match(/\[[A-Za-z<>{}[\]].*?]/gm) !== null) {
        const occurrences = content.match(/\[[A-Za-z<>[\]{}].*]/gm);
        const hashMap = {};
        for (let i = 0; i < occurrences.length; i++) {
          hashMap['function' + i] = occurrences[i];
          content = content.replace(occurrences[i], 'function' + i);
        }
        content = content.replace(/]/g, '\\]');
        for (let i = 0; i < occurrences.length; i++) {
          content = content.replace('function' + i, hashMap['function' + i]);
        }
      } else {
        content = content.replace(/]/g, '\\]');
      }
