...
export class AddTemplateComponent implements OnInit {
  ...
  public add(): void {
    ...
    if (formObj.deviceModel.contentType == ContentTypes.json
      && formObj.templateType.id == TemplateTypes.carche) {

      if (formObj.body.match(/\[[A-Za-z<>\{\}\[\]].*?\]/gm) !== null) {
        const occurences = formObj.body.match(/\[[A-Za-z<>\[\]\{\}].*\]/gm);
        const hashMap = {};
        for (let i = 0; i < occurences.length; i++) {
          hashMap['function' + i] = occurences[i];
          formObj.body = formObj.body.replace(occurences[i], 'function' + i);
        }
        formObj.body = formObj.body.replace(/\]/g, '\\]');
        for (let i = 0; i < occurences.length; i++) {
          formObj.body = formObj.body.replace('function' + i, hashMap['function' + i]);
        }
      } else {
        formObj.body = formObj.body.replace(/\]/g, '\\]');
      }
      ...
      formObj.body = formObj.body.replace(/\{(?![A-Za-z]|[<>\{\}\[\]])/gm, '\\{');
      formObj.body = formObj.body.replace(/\[(?![A-Za-z]|[<>\[\]\{\}])/gm, '\\[');
      const body64 = (btoa(formObj.body));
      formObj.body = body64;
      let template: IConfigTemplateRequest = {
        name: formObj.name,
        body: formObj.body,
        deviceModel: formObj.deviceModel.id,
        // hardcoded event in carche
        version: 1,
      ...
      this.apiConnectService.addcArcheTempalte(JSON.stringify(template)).subscribe(result => {
        this.showSpinner = false;
        this.templateeEdited = result;
        this.activeModal.close({ name: formObj.name, contractid: formObj.contractid, service: formObj.service, id: result['id'] });
        this.activeModal.dismiss({ name: formObj.name, contractid: formObj.contractid, service: formObj.service, id: result['id'] });
      }, error => {
        this.showSpinner = false;
        console.log(error);
        this.messageService.add({ severity: 'error', summary: 'Error Message', detail: 'Unable to create template', sticky: true });
      });
    } else {
      const body64 = (btoa(formObj.body));
      formObj.body = body64;
      const templateJson = JSON.stringify(formObj);
      ...
      this.apiConnectService.addcArcheTempalte(JSON.stringify(template)).subscribe(result => {
        this.showSpinner = false;
        this.templateeEdited = result;
        // must be send also bc company id and bc company nam
        this.activeModal.close({ name: formObj.name, contractid: formObj.contractid, service: formObj.service, id: result['id'] });
        this.activeModal.dismiss({ name: formObj.name, contractid: formObj.contractid, service: formObj.service, id: result['id'] });
      }, error => {
        this.showSpinner = false;
