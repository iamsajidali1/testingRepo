...
export class AddActionModalComponent implements OnInit {
  ...
  public addAction(): void {
    if (this.form.valid) {
      ...
      this.staticHostnameCheckBoxValue = !!this.form.get('staticHostname').value;

      this.canCreateAction = !(this.workflowAttributes.hostname.required && !this.form.get('staticHostname').value);

      if (this.canCreateAction) {
        const validationCommand = 'sh ip int brief';
        const staticHostname = this.form.get('staticHostname')?.value;
        var conditionForStaticHost = staticHostname && typeof staticHostname == 'object' && staticHostname.HOSTNAME
        if (conditionForStaticHost) {
          staticHostname.HOSTNAME = staticHostname.HOSTNAME.trim();
        }

        const formTemplate = new formTemplateSchema(
          this.form.get('name').value,
          null,
          validationCommand,
          this.form.get('description').value,
          this.staticHostnameCheckBoxValue,
          JSON.stringify(this.form.get('confTemplate').value),
          this.form.get("staticHostname").value,
          this.form.get("apiEndpoint").value,
          this.form.get("enabled").value,
          this.form.get("minRollbackTimer").value,
          this.form.get("maxRollbackTimer").value
        )

        this.controllerService.createActionTemplate(
          formTemplate,
          this.form.get('service').value,
          this.form.get('workflow').value['ID'],
          this.form.get('vendorType').value['id'],
          customerId
        ).subscribe(result => {
          ...
          this.activeModal.close({
            ...
            services: this.form.get('service').value
          });
          this.form.reset();
  ...
  checkForConfigTemplate() {
    ...
    this.controllerService.getAttributesForWorkflow(this.form.get('workflow').value.ID).subscribe((attributes) => {
      ...
      attributes.forEach((attribute) => {
        this.updateWorkflowAttribute(
          attribute,
          this.workflowAttributes.configurationTemplate,
          "confTemplate"
        );
        this.updateWorkflowAttribute(
          attribute,
          this.workflowAttributes.hostname,
          "staticHostname",
          this.form.get("customer").value != null
        );
        this.updateWorkflowAttribute(
          attribute,
          this.workflowAttributes.apiEndpoint,
          "apiEndpoint"
        );
      });
  ...
  updateWorkflowAttribute(
    ...
    const attributesStatus = ["required","optional"]
    if(
      attribute["id"] == workflowAttribute.attributId && attributesStatus.includes(attribute["status"])
    ){
      var attributeCondition = !!(attribute["status"] == "required" && additionalCondition) 
      this.form.get(formControlName).setValidators( attributeCondition ? Validators.required : null);
      workflowAttribute.value = true;
      workflowAttribute.required = attributeCondition ? true : false;
    } else if(attribute["id"] == workflowAttribute.attributId){
      this.form.get(formControlName).setValidators(null);
      workflowAttribute.value = false;
      workflowAttribute.required = false;
    }
