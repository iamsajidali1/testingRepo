...
export class FormPropertiesComponent implements OnInit, OnChanges {
  ...
  */
  ...
  isCustomerAndHostnameValid(){
    return this.template['customer'] && 
           this.template['customer']['id'] &&
           this.template['staticHostnameCheckBox'] && 
           this.template['staticHostname'];
  }

  async fetchHostnamesForCustomer(): Promise<void>{
    if (this.isCustomerAndHostnameValid()) {
      try {
  ...
  shouldFilterHostnames(){
    return this.hostnames.length > 0 && 
           this.form.get('vendorType') && 
           this.form.get('services') && 
           this.form.get('services').value['length'] > 0 && 
  ...
  async loadAllHostnames(): Promise<void> {
    ...
    if (this.allHostnames.length <= 0) {
      await this.fetchHostnamesForCustomer();
    } else {
      this.hostnames = this.allHostnames;
    }
    this.loadingHostnames = false;
    if(this.shouldFilterHostnames()){
  ...
  private filterWorkflowAttributes(attributes) {
    ...
    attributes.forEach((attribute) => {
      const updateWorkflowAttribute = (workflowAttribute, formControlName) => {
        ...
        if(attribute["status"] === "required") {
          ...
          this.form.get(formControlName)?.updateValueAndValidity();
          workflowAttribute.required = true;
        }
      };

      const resetWorkflowAttribute = (workflowAttribute, formControlName) => {
        workflowAttribute.value = false;
        this.form.get(formControlName).setValue(null);
        this.form.get(formControlName).updateValueAndValidity();
      }
      
      const handleAttribute = (workflowAttribute, formControlName) => {
        if(attribute["id"] === workflowAttribute.attributId) {
          if(["required","optional"].includes(attribute["status"])){
            updateWorkflowAttribute(workflowAttribute, formControlName);
          } else {
            resetWorkflowAttribute(workflowAttribute, formControlName);
          }
        }
      }

      handleAttribute(this.workflowAttributes.validation, null);
      handleAttribute(this.workflowAttributes.hostname, "staticHostname");
      handleAttribute(this.workflowAttributes.form, null);
      handleAttribute(this.workflowAttributes.configurationTemplate, "confTemplate");
      handleAttribute(this.workflowAttributes.apiEndpoint, "apiEndpoint");
