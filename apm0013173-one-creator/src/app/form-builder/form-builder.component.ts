    ...
    if (question.controlType === 'list') {
      ...
      if (!(question.key in this.form.controls)) {
        ...
        question.columnsType.forEach(q => {
          ...
          } else if (q.required === 'true') {
            const key = q.key;
            array.push(
              this.fb.group({
                [key]: [q.value, Validators.required],
              })
            );
          }
        });
      } else if (question.key in this.form.controls) {                             // edit formArray
        const array = this.form.controls[question.key] as FormArray;
        question.columnsType.forEach(q => {
          let existing = false;
          let index = 0;
          ...
          array.controls.forEach(fg => {
            if (q.key in fg['controls'] || q.oldkey in fg['controls']) {
              existing = true;
              onIndex = index;
            }
            index++;
          ...
          if (!(existing) && q.oldkey === undefined) {            // if new key added
            if (q.required === 'true') {
              const key = q.key;
              array.push(
                this.fb.group({
            ...
            delete q['oldkey'];
          } else if ((existing) && q.oldkey === undefined) {           // key exists but hasnt been changed
            const fg = array.controls[onIndex];
            if (q.required === 'true') {
              fg.get(q.key).setValidators(Validators.required);
              fg.get(q.key).setValue(q.value);
            } else if (q.required === 'false') {
              fg.get(q.key).clearValidators();
              fg.get(q.key).setValue(q.value);
            }
          } else if ((existing) && q.oldkey !== q.key) {           // key has been changed
            const fg = array.controls[onIndex] as FormGroup;
            if (q.required === 'true') {
              fg.removeControl(q.oldkey);
              fg.addControl(q.key, new FormControl(q.value || '', Validators.required));
            } else if (q.required === 'false') {
              fg.removeControl(q.oldkey);
              fg.addControl(q.key, new FormControl(q.value || '', ));
            }
          }
        });
      }
