  ...
  ngOnInit() {
    ...
    if(this.questions.length > 0){
      this.questions.forEach((question)=>{
        this.usedKeyTracking.push(question["key"]);
      })
    }
    ...
    this.usedKeyTracking.forEach(key => {
      if (key) {
        if (key.includes('Hostname')) {
          this.existingHostnameFields.push({ value: key });
        }
      }
    });
    this.getCurrentForm();
    if (this.clonedCarcheTemplateVariables[0] !== undefined && this.clonedCarcheTemplateVariables[0].value == 'empty') {
      this.formToCheck.get('key').disable();
      this.formToCheck.addControl('error', new FormControl('', Validators.required));
    }
    if (this.controlType !== null || this.question !== null) {
      this.subscribeFormKeyChanges();
    }
    ...
    if (this.questionCloned != null && this.questionCloned['options'] != undefined) {
      ...
      if (this.questionCloned['optionsSource']['label'] && this.questionCloned['optionsSource']['value']) {
        this.formDropDown.get('optionsSource').setValue({ label: this.questionCloned['optionsSource']['label'], value: { label: this.questionCloned['optionsSource']['label'], value: this.questionCloned['optionsSource']['value'] } });
      } else {
        this.formDropDown.get('optionsSource').setValue({ label: this.questionCloned['optionsSource'], value: this.questionCloned['optionsSource'] });
      }
      this.formDropDown.get('key').setValue(this.questionCloned['key']);
      this.formDropDown.get('required').setValue(this.questionCloned['required']);
      this.oldkey = this.questionCloned['key'];
    ...
    // if textbox start
    if (this.questionCloned != null && this.questionCloned['controlType'] == 'textbox') {
      this.edit = true;
    ...
    if (this.questionCloned != null && this.questionCloned['columnsType'] != undefined) {
      ...
      this.questionCloned['columnsType'].forEach(column => {
        if (column.controlType == 'textbox') {
          ...
          this.formList.addControl('oldkey' + this.listKey, new FormControl('', Validators.required));
          this.formList.get('oldkey' + this.listKey).setValue(column.key);
          this.formList.addControl('label' + this.listKey, new FormControl('', Validators.required));
          this.formList.get('label' + this.listKey).setValue(column.label);
          this.formList.addControl('placeholder' + this.listKey, new FormControl('',));
          this.formList.get('placeholder' + this.listKey).setValue(column.placeholder);
          this.formList.addControl('validator' + this.listKey, new FormControl('',));
          this.formList.get('validator' + this.listKey).setValue(column.validator);
          this.formList.addControl('key' + this.listKey, new FormControl('', Validators.required));
          this.formList.get('key' + this.listKey).setValue(column.key);
          this.formList.addControl('value' + this.listKey, new FormControl('',));
          this.formList.get('value' + this.listKey).setValue(column.value);
          this.formList.addControl('required' + this.listKey, new FormControl('', Validators.required));
          this.formList.get('required' + this.listKey).setValue(column.required);
          this.formList.addControl('type' + this.listKey, new FormControl('textbox',));
          this.formToCheck.addControl('regex' + this.listKey, new FormControl(false, Validators.required));
          if (this.formToCheck.value['validator' + this.listKey] == 'regexp') {
        ...
        // start if list dropdown
          ...
          this.formList.get('label' + this.listKey).setValue(column.label);
          this.formList.addControl('options' + this.listKey, new FormControl('', Validators.required));
          this.formList.addControl('options' + this.listKey + 'key', new FormControl('', Validators.required));
          this.formList.addControl('required' + this.listKey, new FormControl('', Validators.required));
          this.formList.get('required' + this.listKey).setValue(column.required);
          this.formList.addControl('key' + this.listKey, new FormControl('', Validators.required));
          this.formList.get('key' + this.listKey).setValue(column.key);
          this.formList.addControl('dropdownKey' + this.listKey, new FormControl('', Validators.required));
          this.formList.get('dropdownKey' + this.listKey).setValue(column.key);
          ...
          if (column.optionsSource == "Manual") {
            ...
            column.options.forEach(opt => {
              if (count == 0) {
                let keyPair = [];
                let optKey = 'options' + this.listKey + 'key';
                let optValue = 'options' + this.listKey;
                this.formList.get(optValue).setValue(opt.value);
                this.formList.get(optKey).setValue(opt.key);
                let option = { key: optKey, value: optValue }
                keyPair.push(option);
                this.keyPairs.push(keyPair);
              } else {
                if (this.keyPairs.length - 1 == dropdownsCount) {
                  let optKey = 'options' + this.listKey + this.listOptionsCount + 'key';
                  let optValue = 'options' + this.listKey + this.listOptionsCount;
                  ...
                  this.formList.get(optKey).setValue(opt.key);
                  let option = { key: optKey, value: optValue }
                  this.keyPairs[dropdownsCount].push(option)
                }
                else if (this.keyPairs.length - 1 !== dropdownsCount) {
                  let keyPair = [];
                  let optKey = 'options' + this.listKey + this.listOptionsCount + 'key';
                  let optValue = 'options' + this.listKey + this.listOptionsCount;
                  this.formList.addControl(optValue, new FormControl('', Validators.required));
                ...
                this.rows.forEach(row => {
                  if (row.options == 'options' + this.listKey) {
                    row.optionsss.push('options' + this.listKey + this.listOptionsCount);
                    row.optionsss.push('options' + this.listKey + this.listOptionsCount + 'key');
                  }
                })
                this.listOptionsCount++;
              }
