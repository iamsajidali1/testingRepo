...
function up(queryInterface, DataTypes) {
    return queryInterface.sequelize.query(`create
    or replace view CSS_SR_REPORTS_VIEW as
    select
      tr.ID as ID,
      json_unquote(
        json_extract(
          convert(
            cast(tr.DATA as char charset binary) using latin1
          ),
          '$.transactionId'
        )
      ) as TRANSACTION_ID,
